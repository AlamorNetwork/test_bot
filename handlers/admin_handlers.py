...
# =============================================================================
    ...
    def process_payment_approval(admin_id, payment_id, message):
        ...
        if plan_type == 'fixed_monthly':
            ...
            gb_plan = gb_plan[0] if isinstance(gb_plan, list) and gb_plan else gb_plan
    ...
    def list_profiles_for_management(admin_id, message):
        ...
        if not profiles:
            _bot.edit_message_text(
                "هیچ پروفایلی یافت نشد. لطفاً ابتدا یک پروفایل بسازید.",
                admin_id,
                message.message_id,
                reply_markup=inline_keyboards.get_back_button("admin_profile_management")
            )
            return
